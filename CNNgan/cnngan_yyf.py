...
save_path = r"E:\Pycharmprojects\GAN_again\CNNgan\cnngan_ckpt\1_ckpt"
...
class CNNgannet:
    ...
    def backward(self):
        self.D_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_out, labels=tf.ones_like(self.D_out))) \
                      + tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.G_D_out, labels=tf.zeros_like(self.G_D_out)))
        # 定义损失

        self.D_opt = tf.train.GradientDescentOptimizer(0.01).minimize(self.D_loss, var_list=self.dnet.getparamas())
        # 优化器

        self.G_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.G_D_out, labels=tf.ones_like(self.G_D_out)))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(100000):
            ...
            if epoch % 100 == 0:
                ...
                plt.pause(0.3)
                plt.savefig('E:\Pycharmprojects\GAN_again\CNNgan\cnngan_image\{}.png'.format(str(i).zfill(3)))
